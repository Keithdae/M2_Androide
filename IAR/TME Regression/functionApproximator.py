...
##########################################################################################    
    ...
    def train_GD(self):
        ...
        # Initialize theta_0 
        self.theta = np.random.random(self.numFeatures,)
        ...
        while self.delta >= self.threshold and self.iterationCount < self.maxIter:
            ...
            self.theta = self.theta_old - self.alpha * self.featureOutput(x) * (self.functionApproximatorOutput(x, self.theta) - y)
    ...
    def train_LS(self, data):
        ...
        ## Training Algorithm ##
        #----------------------#
        A = np.dot(self.featureOutput(xData),self.featureOutput(xData).T)
        B = np.dot(self.featureOutput(xData), yData)
    ...
    def train_RLS(self):
        ...
        while self.delta >= self.threshold and self.iterationCount < self.maxIter:
            ...
            ## Training Algorithm ##
            #----------------------#
            A = A + np.outer(self.featureOutput(x), self.featureOutput(x))
            b = b + np.dot(self.featureOutput(x), y)
            self.theta = np.dot(np.linalg.pinv(A),b)
    ...
    def train_RLS2(self):
        ...
        Ainv = np.eye(self.numFeatures,self.numFeatures) * 1
        b = np.zeros(self.numFeatures,)
        ...
        while self.delta >= self.threshold and self.iterationCount < self.maxIter:
            ...
            Ainv = Ainv - (Ainv.dot(np.outer(self.featureOutput(x),self.featureOutput(x))).dot(Ainv)) / (1. + self.featureOutput(x).T.dot(Ainv).dot(self.featureOutput(x)))            
            b = b + np.dot(self.featureOutput(x), y)
