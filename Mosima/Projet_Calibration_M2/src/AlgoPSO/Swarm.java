...
public class Swarm implements Iterable<Particle> {
	 ...
	 */
	public double[] evaluate(double fitnessTarget) {
		...
		for (int i = 0; i < particles.length; i++) {
			...
			// Evaluate particle
			double[] results = fitnessFunction.evaluate(particles[i]);
	 ...
	 */
	public double[] evolve(double fitnessTarget) {
		// Initialize (if not already done)
		if (particles == null) init();

		double[] results = evaluate(fitnessTarget); // Evaluate particles
